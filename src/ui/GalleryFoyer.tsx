 ...
 */
export function GalleryFoyer({ station, index, total, onPrevious, onNext, onEnter }: Props) {
  const cover = deckCovers.get(station.id) ?? station.heroImage;

  return (
    <section className="gallery-foyer" aria-label={`Ruang pilihan: ${station.label}`}>
      ...
      <div className="gallery-foyer-copy">
        <span className="gallery-foyer-kicker">
          <Glyph glyph={station.glyph} size={13} /> PORT / BILIK PILIHAN
        </span>
        <h1>{station.label}</h1>
        <p>{station.tagline}</p>
        <div className="gallery-foyer-actions">
          <button type="button" className="btn btn-primary" onClick={onEnter}>
            Masuk ke ruang <span aria-hidden="true">↗</span>
          </button>
          <div className="gallery-foyer-pager" aria-label="Pilih ruang lain">
            <button type="button" className="foyer-arrow" onClick={onPrevious} aria-label="Ruang sebelumnya">
              ←
            </button>
            <button type="button" className="foyer-arrow" onClick={onNext} aria-label="Ruang seterusnya">
              →
            </button>
          </div>
        </div>
